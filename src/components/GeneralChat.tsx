...
const GeneralChat: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadHistory = async () => {
            const initialMessages = await getCompanionChatHistory();
    ...
    // Save companion chat on unmount (navigating away within the app)
    useEffect(() => {
        return () => {
            const msgsToSave = messagesRef.current.map(m => ({ ...m, image: m.image?.startsWith('blob:') ? undefined : m.image }));
            saveCompanionChatHistory(msgsToSave).catch(console.error);
        };
    }, []);

    // Safety net: also save when browser tab is closed / refreshed
    useEffect(() => {
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                if ((chunk as any).candidates?.[0]?.groundingMetadata?.groundingChunks) {
                    for (const groundingChunk of (chunk as any).candidates[0].groundingMetadata.groundingChunks) {
            ...
            // Auto-save after every complete AI response (survives page refresh)
            if (responseText) {
                const msgsToSave = messagesRef.current.map(m => ({ ...m, image: m.image?.startsWith('blob:') ? undefined : m.image }));
                saveCompanionChatHistory(msgsToSave).catch(console.error);
            }
