...
const History: React.FC = () => {
    ...
    useEffect(() => {
        const loadHistory = async () => {
            const [plansData, tutorData, companionData] = await Promise.all([
                getStudyPlanHistory(),
                getTutorChatHistory(),
                getCompanionChatHistory()
            ]);
            setPlans(plansData);
            setTutorChats(tutorData.sort((a, b) => new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime()));
            setCompanionChat(companionData);
        };
        loadHistory();
    }, []);
