...
const TutorChat: React.FC = () => {
    ...
    // Safety net: also save when browser tab is closed / page refreshed
    useEffect(() => {
    ...
    const handleStartSession = useCallback(async () => {
        const history = await getTutorChatHistory();
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            ...
            if (sessionDetailsRef.current.id && responseText) {
                const currentMsgs = messagesRef.current;
                const msgsToSave = currentMsgs.map(m => ({ ...m, image: m.image?.startsWith('blob:') ? undefined : m.image }));
                saveTutorChatSession({
                    id: sessionDetailsRef.current.id,
                    curriculum: sessionDetailsRef.current.curriculum,
                    subject: sessionDetailsRef.current.subject,
