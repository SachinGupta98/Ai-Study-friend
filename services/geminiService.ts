...
export const getTutorResponseStream = async (
    ...
    try {
        // FIX: The `sendMessageStream` method expects the `message` property to be an array of parts, not an object containing a `parts` property.
...
const _summarizeHistory = async (historyToSummarize: ChatMessage[]): Promise<string> => {
    ...
    try {
        const response = await model.generateContent({
            model: 'gemini-3-flash-preview', // Flash is efficient for summarization
...
export const getGeneralChatResponseStream = async (
    ...
    const systemInstruction = `You are "Vidya AI" in a friendly, conversational mode. 
        Your role is to be a supportive and empathetic companion. You can chat with students about their day, hobbies, real-life problems, or any general topic.
        Your tone should be encouraging, positive, and non-judgmental. 
        You are not a formal tutor, but you are also a knowledgeable and encouraging career counselor. When asked for career guidance, provide information on potential career paths, required skills (both technical and soft), and suggest next steps or resources for learning.
    ...
    // Detect if the query is career-related to enable search grounding
    const isCareerQuery = /career|job|skill|path|salary|market|guidance/i.test(message);
...
export const generateMotivationalMessage = async (progress: number, subject: string): Promise<string> => {
    ...
    try {
        const response = await model.generateContent({
            model: 'gemini-3-flash-preview', // Flash is suitable for this simple task
            contents: prompt,
            config: {
                temperature: 0.9
            }
